    ...
    : QWidget(parent)
{
    ...
    if (QSqlDatabase::drivers().isEmpty())
        QMessageBox::information(this, tr("No database drivers found"),
                                 tr("This application requires at least one Qt database driver. "
                                    "Please check the documentation how to build the "
                                    "Qt SQL plugins."));

    emit statusPosted(tr("Ready."));
}
...
void Browser::exec()
{
    ...
    if (model->lastError().type() != QSqlError::NoError)
        emit statusPosted(model->lastError().text());
    else if (model->query().isSelect())
        emit statusPosted(tr("Query OK."));
    else
        emit statusPosted(tr("Query OK, number of affected rows: %1").arg(model->query().numRowsAffected()));
                                 ...
                                 const QString &user, const QString &passwd, int port)
{
    ...
    QSqlError err;
    QSqlDatabase db = QSqlDatabase::addDatabase(driver, QString("DB%1").arg(++cCount));
    ...
    if (!db.open(user, passwd)) {
        err = db.lastError();
        db = QSqlDatabase();
        QSqlDatabase::removeDatabase(QString("DB%1").arg(cCount));
    }
...
void Browser::addDatabase()
{
    QString dbName = QFileDialog::getOpenFileName(this, tr("Open File"), QString(),
                                                  tr("SQLite Files (*.sqlite)"), 0, 0);
    if (!dbName.isEmpty()) {
        QSqlError err = addConnection("QSQLITE", dbName, NULL, NULL, NULL, 0);
